...
class GetRAGOutputPresenter(GetRAGOutputPresenterInterface):
    ...
    def get_output(self, input_query: str) -> Optional[Tuple[str, List[str]]]:
        ...
        text_preprocessing_interactor = TextProcessingInteractor()

        profanity_exists = \
            text_preprocessing_interactor.profanity_check(text=input_query)
        if profanity_exists:
            return PROFANITY_RESPONSE

        print("\t Checking Malicious code\t")
        if firewall.check_malicious_code_input(prompt_text=input_query):
            return NO_CODE
        print("\t Checking Malicious prompt\t")
        if firewall.check_malicious_string(prompt_text=input_query):
            return POLITE_REFUSAL

        print("\t Performing text processing\t")
        input_query = \
            text_preprocessing_interactor.perform_text_processing(
                text=input_query)
        ...
        prompt_template = prompt_template_interactor.get_prompt_template(
        ...
        try:
            is_valid_query = validate_text_interactor.validate_results(
        ...
        qa_retrieval = retriever_interactor.get_retrievalqa(
            retriever=retriever,
            llm=local_llm,
        ...
        bot_response_processed = (
            text_preprocessing_interactor.text_post_processing(
                text=bot_result,
